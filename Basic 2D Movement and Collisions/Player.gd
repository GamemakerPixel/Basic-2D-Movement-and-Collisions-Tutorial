...
export (int) var speed #export - allows this variable to be edited in the inspector, (int) - states that this variable is an integer and will only allow the given value to be an integer
var Velocity = Vector2() #Vector2s store an x and y value, in this case, the left and right velocity (x) and the upward and downward velocity (y)
